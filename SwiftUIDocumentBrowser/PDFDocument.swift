...
struct PDFDocument: FileDocument {
    ...
    var referenceDocument: PSPDFKit.Document
    ...
    init(data: Data = Data(), contentType: UTType = .pdf) {
        guard let document = PDFDocument.createDocument(data: data, contentType: contentType) else {
            ...
            referenceDocument = PSPDFKit.Document()
            return
        }
    ...
    private static func createDocument(data: Data = Data(), contentType: UTType) -> PSPDFKit.Document? {
